...
if mp.current_process().name == 'MainProcess':
    ...
    import viper.src.model_definitions as model_definitions
    # Create a list of all the defined models
    list_models = [m[1] for m in inspect.getmembers(model_definitions, inspect.isclass)
                   if issubclass(m[1], model_definitions.BaseModel) and m[1] != model_definitions.BaseModel]
...
def make_fn(model_class, process_name, counter):
    ...
    num_gpus = torch.cuda.device_count() if torch.cuda.device_count() > torch.mps.device_count() else torch.mps.device_count()
    gpu_number = counter % num_gpus if num_gpus > 0 else 0
    
    if num_gpus < 1:
        warnings.warn(
            f'No GPUs available. Running {model_class.name} on CPU. This may be slow.',)
